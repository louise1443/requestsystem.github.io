...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($name) || empty($date_of_birth) || empty($civil_status) || empty($address) || empty($gender) || empty($contact)) {
        ...
        $stmt = $connection->prepare("
            INSERT INTO fencing_permit 
            (user_id, admin_id, name, date_of_birth, civil_status, address, gender, contact, requested_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        ...
        if (!$stmt) {
            ...
            if ($stmt->execute()) {
                $successMessage = "✅ Fencing Permit request submitted successfully.
...
Swal.fire({
    ...
    window.location.href = "requestfencingpermit.php";
});
<?php endif; ?>
