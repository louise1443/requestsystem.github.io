...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($address) || empty($gender) || empty($civil_status) || empty($contact) || empty($business_name)) {
        ...
        $stmt = $connection->prepare("
            INSERT INTO cert_for_solid_waste_management
            (user_id, admin_id, name, business_name, address, gender, civil_status, contact, date_applied)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        $stmt->bind_param("iissssss",$userId, $admin_id, $name, $business_name, $address, $gender, $civil_status, $contact);

        if ($stmt->execute()) {
            $successMessage = "✅ Certification request submitted successfully.
                                Always check your notification bell for new update!";
            $name = $address = $business_name = $gender = $civil_status = $contact = "";
        } else {
...
<div class="main">
    <div class="container">
        <h2>Certification for Solid Waste Management</h2>
