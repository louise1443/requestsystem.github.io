...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($address) || empty($gender) || empty($civil_status) || empty($contact) || empty($purpose)) {
        ...
        $stmt = $connection->prepare("
            INSERT INTO barangay_construction_clearance
            (user_id, admin_id, name, address, gender, civil_status, contact, purpose, date_applied)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        $stmt->bind_param("iissssss", $userId, $admin_id, $name, $address, $gender, $civil_status, $contact, $purpose);

        if ($stmt->execute()) {
            $successMessage = "✅ Barangay Construction Clearance submitted successfully.
                                Always check your notification bell for new update!";
            $name = $address = $gender = $civil_status = $contact = $purpose = "";
        } else {
...
<div class="main">
    <div class="container">
        <h2>Barangay Construction Clearance</h2>
        <form method="POST" action="">
            <label>Full Name</label>
            <input type="text" name="name" placeholder="Enter Full Name" value="<?= htmlspecialchars($name); ?>" required>
            ...
            <label>Address</label>
                <select name="address" required>
                    ...
                    <option <?= ($address=='Purok Kalubihan')?'selected':'' ?>>Purok Kalubihan</option>
            ...
            <input type="text" placeholder="Enter Contact Number" name="contact" value="<?= htmlspecialchars($contact); ?>" maxlength="11" required>
...
Swal.fire({
    ...
    window.location.href = "requestbarangayconstructionclearance.php";
});
<?php endif; ?>
