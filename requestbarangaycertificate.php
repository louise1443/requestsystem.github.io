...
// ✅ Keep input values
$name = $date_of_birth = $civil_status = $address = $gender = $contact = "";
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($name) || empty($date_of_birth) || empty($civil_status) || empty($address) || empty($gender) || empty($contact)) {
        $errorMessage = "⚠ All fields are required!";
    } elseif (!preg_match('/^09\d{9}$/', $contact)) {
        $errorMessage = "⚠ Contact must be exactly 11 digits and start with '09'.";
    } else {
        $stmt = $connection->prepare("
            INSERT INTO barangay_certificate 
            (user_id, admin_id, name, date_of_birth, civil_status, address, gender, contact, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        ...
        if (!$stmt) {
            ...
            $stmt->bind_param("iissssss", $userId, $admin_id, $name, $date_of_birth, $civil_status, $address, $gender, $contact);
            if ($stmt->execute()) {
                $successMessage = "✅ Barangay Certificate request successfully.
...
<body>
    ...
    <script>
        ...
        Swal.fire({
            ...
            window.location.href = "requestbarangaycertificate.php";
        });
        <?php endif; ?>
