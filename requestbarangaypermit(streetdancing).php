<?php
// barangaypermit(streetdancing).php
session_start();
...
$admin_id = $_SESSION["admin_id"] ?? 1; // default admin ID if missing
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($fullname) || empty($address) || empty($civil_status) || empty($dob) || empty($date_to_hold) || empty($contact) || empty($chapel_name) || empty($start_end)) {
        ...
        if ($stmt->execute()) {
            $successMessage = "✅ Barangay Permit Street Dancing submitted successfully.
...
<div class="main">
    <div class="container">
        <h2>Barangay Permit (Street Dancing)</h2>
        <form method="POST" action="">
            ...
            <label>Date Start to Hold</label>
            <input type="date" name="date_to_hold" value="<?= htmlspecialchars($date_to_hold); ?>" required>
...
Swal.fire({
    ...
    window.location.href = "requestbarangaypermit(streetdancing).php";
});
<?php endif; ?>
