<?php 
session_start(); // ✅ Needed for session access
...
// ✅ Default admin_id (optional if needed)
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($fullname) || empty($address) || empty($gender) || empty($civil_status) || empty($dob) || empty($place_of_birth) || empty($contact) || empty($purpose)) {
        $errorMessage = "⚠ All fields are required!";
    } elseif (!preg_match('/^09\d{9}$/', $contact)) {
        $errorMessage = "⚠ Contact must be 11 digits and start with '09'.";
    } else {
        // ✅ Insert the application
        $stmt = $connection->prepare("INSERT INTO barangay_clearance 
            (user_id, admin_id, fullname, address, gender, civil_status, dob, place_of_birth, contact, purpose, date_applied) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");

        $stmt->bind_param("iissssssss", $userId, $admin_id, $fullname, $address, $gender, $civil_status, $dob, $place_of_birth, $contact, $purpose);

        if ($stmt->execute()) {
            $successMessage = "✅ Barangay Clearance application submitted successfully.
                                Always check your notification bell for new update!";
            // ✅ Clear form after success
            $address = $dob = $contact = $purpose = "";
        } else {
...
<div class="main">
    <div class="container">
        ...
        <form method="POST" action="">
            ...
            <input type="text" name="fullname" placeholder="Enter Full Name" value="<?= htmlspecialchars($fullname); ?>" required>
            ...
            <input type="text" name="place_of_birth" placeholder="Enter Place of Birth" value="<?= htmlspecialchars($place_of_birth); ?>" required>

            <label>Contact Number</label>
            <input type="text" name="contact" placeholder="Enter Contact Number" value="<?= htmlspecialchars($contact); ?>" maxlength="11" required>

            <label>Purpose</label>
            <textarea name="purpose" rows="3" required><?= htmlspecialchars($purpose); ?></textarea>
...
Swal.fire({
    ...
    window.location.href = "/system/requestclearance.php";
});
<?php endif; ?>
