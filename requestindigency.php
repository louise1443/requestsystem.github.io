...
$address = $civil_status = $dob = $contact = $reason = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // ✅ Validation
    if (empty($fullname) || empty($address) || empty($civil_status) || empty($dob) || empty($contact) || empty($reason)) {
        ...
        $stmt = $connection->prepare("INSERT INTO barangay_indigency 
            (user_id, admin_id, fullname, address, civil_status, dob, contact, reason, date_applied) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");

        $stmt->bind_param("iissssss", $userId, $admin_id, $fullname, $address, $civil_status, $dob, $contact, $reason);

        if ($stmt->execute()) {
            $successMessage = "✅ Barangay Indigency application submitted successfully.
                                Always check your notification bell for new update!";
            // ✅ Clear form after success
            $address = $dob = $contact = $reason = "";
        } else {
